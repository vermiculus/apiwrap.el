...
generates (and in fact, you /must/ use this to compile your file).  Once
you use =API-Wrap.el= to compile your code, you don't need to generate
the macros again!  However, you /do/ still need to load it in
application code to use support functions that have to evaluate at
runtime (like ~apiwrap-plist->alist~).
